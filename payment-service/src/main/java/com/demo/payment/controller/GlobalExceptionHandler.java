...
import com.demo.payment.exception.FeignClientException;
import com.demo.payment.utils.ResponseEntityUtils;
...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<APIResponse<?>> handleFeignClientException(FeignClientException ex, WebRequest request) {
        this.logError("Feign client exception: ", ex, request);
        FeignClientException exception = ex.getCause() == null ? ex : (FeignClientException) ex.getCause();
        APIResponse<Object> apiResponse = createDefaltApiResponse(ex, request, ex.getCode());
        apiResponse.setCode(exception.getCode());
        apiResponse.setData(ex.getData());
        return ResponseEntityUtils.clientError(apiResponse);
    }
